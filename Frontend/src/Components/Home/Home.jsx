...
const Home = () => {
    ...
    useEffect(() => {
        ...
        let result =await  fetch(`${process.env.REACT_APP_BACKEND_URL}http://localhost:3000/api/toppicks`)
    ...
    useEffect(() => {
        const fetchAgents = async () => {
            try{
                let result = await fetch(`${process.env.REACT_APP_BACKEND_URL}http://localhost:3000/api/agents`)
                result = await result.json()
                setAgents(result);
            }catch(error){
